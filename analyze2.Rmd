...
# use the population cagr instead for the scatter and us abline 1
...
scatter_df |>
    ...
    ggplot(aes(x=population_cagr, y=fuzzy_cagr, label=short_name)) +
    geom_hline(yintercept = avg_nom_cagr, linetype='dashed') +
    geom_vline(xintercept = avg_pop_cagr, linetype='dashed') +
    geom_point( alpha = 0.5 ) +
    geom_point(data = . %>% filter(GeoFIPS %in% top_cities_2020), color='red') +
    geom_abline(
                intercept = avg_nom_cagr - avg_pop_cagr, slope=1
    ) +
    geom_abline(intercept = avg_nom_cagr + avg_pop_cagr, slope=-1) +
    geom_label_repel(data = . %>% filter(GeoFIPS %in% top_cities_2020)) +
    scale_x_continuous(labels = scales::percent_format()) +
    scale_y_continuous(labels = scales::percent_format()) +
    labs(
         x = "Population Growth CAGR (2010-2020)",
         y = "Mean Wage CAGR (2010-2020)",
         title = "Changes in Nominal Income vs Net Migration Rate",
         caption = "Source: BEA, University of Wisconsin-Madison, 2024"
    )

ggsave('imgs/45-lines-popcagr.png' , width=10, height=10)
...
scatter_df |>
    filter(net_migrant_rate < 0.4) |>
    ggplot(aes(x=net_migrant_cagr, y=fuzzy_cagr, label=short_name)) +
    geom_hline(yintercept = avg_nom_cagr, linetype='dashed') +
    geom_vline(xintercept = avg_net_migrant_cagr, linetype='dashed') +
    geom_point( alpha = 0.5 ) +
    geom_point(data = . %>% filter(GeoFIPS %in% top_cities_2020), color='red') +
    geom_abline(
                intercept = avg_nom_cagr - avg_net_migrant_cagr, slope=1
    ) +
    geom_abline(intercept = avg_nom_cagr + avg_net_migrant_cagr, slope=-1) +
    geom_label_repel(data = . %>% filter(GeoFIPS %in% top_cities_2020)) +
    scale_x_continuous(labels = scales::percent_format()) +
    scale_y_continuous(labels = scales::percent_format()) +
    labs(
         x = "Net Migration CAGR (2010-2020)",
         y = "Mean Wage CAGR (2010-2020)",
         title = "Changes in Nominal Income vs Net Migration Rate",
         caption = "Source: BEA, University of Wisconsin-Madison, 2024"
    )

ggsave('imgs/45-lines-net-migrant-cagr.png' , width=10, height=10)
...
create_shift_chart("San Jose")
create_shift_chart("San Francisco")
create_shift_chart("Los Angeles")
create_shift_chart("Phoenix")
create_shift_chart("Chicago")

create_shift_chart("Raleigh")

create_shift_chart("New York")
ggsave('imgs/new-york-sectoral-employment.png', width=10, height=6)

gen_city_patches("Chicago")
gen_city_patches("New York")
# now look into zillow for nyc

city_merged |>
    select(sector, full_sector_title) |> unique()
